...
**RBAC**: `uami-storage-cmk` requires Key Vault Crypto Service Encryption User on the Key Vault. This role assignment is documented in ADR-004 section `CMK Identity Inventory and Key Vault RBAC Assignments`.
...
**RBAC**: `sami-search` requires Key Vault Crypto Service Encryption User on the Key Vault. This role assignment is documented in ADR-004 section `CMK Identity Inventory and Key Vault RBAC Assignments`.

**Key Vault access path:** AI Search accesses Key Vault for CMK operations through a shared private link: a search-service-managed private endpoint provisioned by AI Search and approved on the Key Vault side. This is distinct from the platform VNet private endpoint used by compute-layer services. AI Search does not traverse the customer VNet private endpoint and does not require the trusted-services firewall bypass. RBAC (Key Vault Crypto Service Encryption User assigned to sami-search) remains the primary access control. Shared private links are a premium feature with per-link usage-based pricing; cost is tracked under the AI Search resource. Provisioning and approval workflow is validated during the IaC spike (see Follow-ups).
...
### Azure AI Foundry (Cognitive Services): CMK
...
### Azure Service Bus: CMK

**Encryption scope**: Namespace-level encryption at rest using a customer-managed key from Azure Key Vault.

**Decision**: Customer-managed keys.

**Rationale**: Azure Service Bus Premium supports customer-managed keys. The platform already provisions Key Vault with CMK for Azure SQL and Storage. The identity model and key rotation mechanisms are in place. Applying CMK to Service Bus is a single key, one identity assignment, and one configuration setting on existing infrastructure.

Service Bus carries work dispatch, coordination messages, and review routing events. Message payloads are operationally scoped today. But the platform is designed to evolve. New use cases will be added. Message content will change. Maintaining an MMK exception for Service Bus creates a boundary condition that depends on someone assessing payload sensitivity every time a use case or message schema changes. That assessment will not happen reliably.

Applying CMK consistently across all services that support it eliminates the need to track which services carry sensitive data and which do not. The encryption posture is uniform. There is no exception to monitor, no boundary condition to enforce, and no governance finding when a message payload grows beyond its original scope.

### Log Analytics Workspace: MMK
...
Azure AI Foundry has native integration with Microsoft Purview (GA, documented December 2025). When enabled through the Foundry Control Plane or Defender for Cloud, Purview provides auditing, data classification, sensitivity label enforcement, DLP, insider risk management, eDiscovery, and retention for Foundry AI interactions including prompts and responses. This integration requires pay-as-you-go Purview billing (audit is included in the base license). The platform architecture accommodates this integration; enablement and policy configuration are scoped to the Purview design workstream, not this ADR.
...
**ADR-004, Identity and RBAC**: All CMK identity definitions and Key Vault role assignments are documented in ADR-004 section `CMK Identity Inventory and Key Vault RBAC Assignments`. ADR-005 references those assignments and documents the rationale for why each identity needs Key Vault access. The Fabric Platform CMK service principal, Foundry CMK identity, and Azure SQL CMK identity rows exist in that ADR-004 section; ADR-005 owns the explanation of why they are required and how they function. The Foundry CMK role discrepancy, Key Vault Crypto User vs Key Vault Crypto Service Encryption User, is documented in section 6.3 and reflected in ADR-004.
